...
function ItemDrawerUI({
  ...
  useEffect(
    function syncDrawerOpenWithSelectedItem() {
  ...
  return (
    <div ref={ref}>
      <DrawerContainer isOpen={isOpen} onClick={handleClick}>
        <div className="flex items-center justify-between">
          <Row>
            <DrawerIconContainer selected={isOpen}>{icon}</DrawerIconContainer>
            <DrawerItemText selected={isOpen} text={text} />
          </Row>
          {isOpen ? (
      ...
      >
        {items.map(function ({ event, href, text: itemText, urlToBeSelected }) {
          ...
          return (
            ...
            >
              <div
                className={`relative ml-4 flex flex-col gap-1 px-5 py-1.5 text-sm
...
function ItemDrawerImpl({ ...props }: Omit<Props, 'urlToBeSelected'>) {
  ...
  return <ItemDrawerUI {...allProps} />
}

export const ItemDrawer = (props: ItemDrawerProps) => (
  <Suspense fallback={<ItemDrawerUI {...props} />}>
    <ItemDrawerImpl {...props} />
  </Suspense>
)
