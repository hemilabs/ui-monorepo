...
export const getRewardPeriodsForDay = async function (
  client: Client,
  rewardToken: Address,
  timestamp: bigint,
): Promise<bigint> {
  ...
  return readContract(client, {
    abi: veHemiRewardsAbi,
    address: veHemiRewardsAddress,
    args: [rewardToken, timestamp],
    functionName: 'rewardPeriods',
  })
}

export const getStartingTimestamp = async function (
  client: Client,
): Promise<bigint> {
  if (!client.chain) {
    throw new Error('Client chain is not defined')
...
export const getMaxConfiguredReward = async function (
  client: Client,
  rewardToken: Address,
): Promise<bigint> {
  if (!client.chain) {
