    ...
    <div className="flex items-center gap-x-2 rounded-lg bg-zinc-50 p-2 text-xs font-medium md:text-sm">
      <div
        className={`flex h-6 w-6 items-center justify-center rounded-full ${
          completed ? 'bg-green-500/10' : 'bg-gray-200'
        }`}
      >
        {completed ? <CheckIcon /> : icon}
      </div>
      <p className="mr-auto text-zinc-400">{text}</p>
      {'gas' in props && 'symbol' in props && !withdrawTxHash ? (
        <>
  ...
  return (
    <Card>
      ...
      <SubSection
        ...
        value={canWithdraw ? toWithdraw : '0'}
      />
      <Step
        completed={progress > WithdrawProgress.WITHDRAWING}
        gas={gas}
        icon={<PlaneIcon />}
        symbol={gasSymbol}
      ...
      <Step
        completed={progress >= WithdrawProgress.READY_TO_PROVE}
        icon={<WaitIcon />}
      ...
      />
      <Step
        completed={progress >= WithdrawProgress.WAITING_FOR_CLAIM_ENABLED}
        icon={<ProveIcon />}
        text={t('prove-withdrawal')}
      />
      <VerticalLine status={getProgressStatus(WithdrawProgress.PROVING)} />
      <Step
        completed={progress >= WithdrawProgress.READY_TO_CLAIM}
        icon={<WaitIcon />}
        text={t('wait-hours', { hours: ExpectedProofWaitTimeHours })}
      />
      <VerticalLine
        status={getProgressStatus(WithdrawProgress.READY_TO_PROVE)}
      />
      <Step
        completed={progress === WithdrawProgress.CLAIMED}
        icon={<PlaneIcon />}
