...
import { Chain as ChainComponent } from './_components/chain'
import { TxLink } from './_components/txLink'
import { TxStatus } from './_components/txStatus'
import { TxTime } from './_components/txTime'
import { WithdrawAction } from './_components/withdrawAction'
...
const Header = ({ text }: { text?: string }) => (
  <span className="block text-left text-sm font-medium text-neutral-400">
    {text}
  </span>
)
              ...
              rows.map(row => (
                <tr key={row.id}>
                  {row.getVisibleCells().map(cell => (
                    <td className="py-2 text-neutral-700" key={cell.id}>
                      {flexRender(
