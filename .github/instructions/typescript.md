...
Note that `@tsconfig/node24` must be installed in the `package.json`. Install the same exact version that is already part of the [package-lock file](../../package-lock.json). If multiple versions are found, use the newest.
