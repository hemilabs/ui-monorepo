...
// use hemi-viem because we need sourceId property, that's not available in viem
import { hemi, hemiSepolia } from 'hemi-viem'
import { type Chain, isAddress, isAddressEqual, zeroAddress } from 'viem'
import { mainnet, sepolia } from 'viem/chains'

import { NativeTokenSpecialAddressOnL2 } from './nativeTokens'
import { Extensions, type Token } from './types'
...
const tinyPartition = <T>(arr: T[], predicate: (t: T) => boolean) =>
  [arr.filter(predicate), arr.filter(t => !predicate(t))] as const
...
const whitelistTunnel = function (hemiChain: Chain, addresses: string[]) {
  ...
  const [nativeTokens, erc20Tokens] = tinyPartition(addresses, isNativeAddress)
...
export const tunnelWhiteList: Partial<
  Record<Token['chainId'], Record<string, Pick<Extensions, 'tunnel'>>>
> = {
