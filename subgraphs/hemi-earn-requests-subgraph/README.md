...
- `ENVIO_RPC_URL_BNB`, `ENVIO_RPC_URL_ETH`, `ENVIO_RPC_URL_HEMI` — the RPC URLs used to read the balances on each chain. Join several URLs with `+` (for example `https://a.example+https://b.example`) use them as viem fallbacks. The public RPCs can reject the historical reads, so set an archive URL on each chain.
