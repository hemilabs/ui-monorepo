...
import { recoverRedeem } from '../../../src/actions/wallet/recoverRedeem'
...
describe('recoverRedeem', function () {
  it('emits "unexpected-error" when wallet client chain is not defined', async function () {
    ...
    const { emitter, promise } = recoverRedeem({
      ...validParameters,
  ...
  it('happy path: signs, waits receipt, emits succeeded', async function () {
    ...
    const { emitter, promise } = recoverRedeem(validParameters)
